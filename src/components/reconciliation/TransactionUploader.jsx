  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <input id="posFile" type="file" accept=".csv" onChange={(e) => handleFile('posTransactions', e.target.files[0])} />
      ...
      <button onClick={handleSubmit} disabled={isProcessing} className="bg-blue-600 text-white px-4 py-2 rounded">
        ...
        {Object.entries(previews).map(([key, rows]) => (
          <div key={key} className="text-xs border p-2 rounded">
            ...
            <pre className="whitespace-pre-wrap">
              {rows.map((r, i) => JSON.stringify(r)).join('\n')}
            </pre>
          </div>
        ))}
      </div>
