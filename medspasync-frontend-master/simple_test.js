  ...
  return new Promise((resolve, reject) => {
    ...
    const options = {
      ...
      headers: {
        ...
        'User-Agent': 'MedSpaSync-Test/1.0'
      }
    };
...
async function testBackendHealth() {
  try {
    ...
    if (response.status === 200) {
      addResult('backend', 'Health Check', true, `Status: ${response.status}`);
...
async function testBackendAuth() {
  try {
    ...
    const registerResponse = await makeRequest(`${config.backend}/api/auth/register`, 'POST', registerData);
    ...
    const loginResponse = await makeRequest(`${config.backend}/api/auth/login`, 'POST', loginData);
...
async function testAiApi() {
  try {
    ...
    const healthResponse = await makeRequest(`${config.aiApi}/health`);
    ...
    const reconciliationResponse = await makeRequest(`${config.aiApi}/reconcile`, 'POST', reconciliationData);
...
async function testFrontend() {
  try {
    ...
    if (response.status === 200) {
      ...
      if (response.body.includes('MedSpaSync') || response.body.includes('Next.js')) {
